...
def main():
    ...
    # BitFit setup: only during training
    if training_args.do_train:
        ...
        print(
            f"BitFit: trainable params = {trainable_params} "
            f"({100 * trainable_params / total_params:.4f}% of {total_params})"
        )
    ...
    # Reset CUDA peak memory stats before any heavy work
    if torch.cuda.is_available():
        torch.cuda.reset_peak_memory_stats()

    # Training preprocessing
    def prepare_train_features(examples):
        # Remove left whitespace from questions
    ...
    # Log peak GPU memory usage for any run that actually did work
