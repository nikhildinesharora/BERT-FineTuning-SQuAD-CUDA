...
def main():
    ...
    # ----- B3: partial layer freezing (only if training) -----
    if training_args.do_train:
        # Freeze embeddings
        for param in model.bert.embeddings.parameters():
            param.requires_grad = False

        # Freeze bottom 8 encoder layers: layer[0]..layer[7]
        for layer in model.bert.encoder.layer[:8]:
            for param in layer.parameters():
                param.requires_grad = False

        # Count trainable params for reporting
        trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
        total = sum(p.numel() for p in model.parameters())
        print(f"Trainable params: {trainable} / {total} ({100 * trainable / total:.2f}%)")

    # Reset CUDA peak stats before actual training/eval to measure this run
    ...
    if not isinstance(tokenizer, PreTrainedTokenizerFast):
        raise TypeError(
            ...
            "See the Transformers docs for supported models."
        )
    ...
    # Log GPU memory usage for this run
