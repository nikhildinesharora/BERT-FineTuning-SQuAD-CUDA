...
def main():
    ...
    if training_args.torch_compile:
        logger.info(
            f"torch.compile is ENABLED "
            f"(backend={training_args.torch_compile_backend}, mode={training_args.torch_compile_mode})"
        )
    ...
    if not isinstance(tokenizer, PreTrainedTokenizerFast):
        raise TypeError(
            ...
            "Check https://huggingface.co/transformers/index.html#supported-frameworks "
    ...
    def prepare_train_features(examples):
        # Strip left whitespace in questions
        examples[question_column_name] = [q.lstrip() for q in examples[question_column_name]]

        # Tokenize examples with truncation and stride
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            if len(answers["answer_start"]) == 0:
                ...
                # End token index of current span in the text.
                token_end_index = len(input_ids) - 1
                while sequence_ids[token_end_index] != (1 if pad_on_right else 0):
                    token_end_index -= 1

                # If answer is not fully inside the span, label CLS.
