...
"""
Fine-tuning the library models for question answering using a slightly adapted
...
def main():
    ...
    if not isinstance(tokenizer, PreTrainedTokenizerFast):
        raise TypeError(
            ...
            "Check the supported models table in the Transformers docs."
        )
    ...
    def prepare_train_features(examples):
        # Remove left whitespace on questions
        examples[question_column_name] = [q.lstrip() for q in examples[question_column_name]]

        # Tokenize with truncation/stride
    ...
    def post_processing_function(examples, features, predictions, stage="eval"):
        ...
        if data_args.version_2_with_negative:
            ...
            formatted_predictions = [
                {"id": str(k), "prediction_text": v} for k, v in predictions_dict.items()
            ]
    ...
    # Log peak GPU memory (for *-evalmem runs etc.)
